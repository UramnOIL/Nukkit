...
class JungleTreePopulator @JvmOverloads constructor(private val type: Int = BlockSapling.JUNGLE) : Populator() {
	...
	override fun populate(level: ChunkManager, chunkX: Int, chunkZ: Int, random: NukkitRandom, chunk: FullChunk) {
		...
		for (i in 0 until amount) {
			...
			NewJungleTree(4 + random.nextBoundedInt(7), 3).generate(level, random, v.setComponents(x.toDouble(), y.toDouble(), z.toDouble()))
		}
	}
