...
class SwampTreePopulator @JvmOverloads constructor(private val type: Int = BlockSapling.OAK) : Populator() {
	...
	override fun populate(level: ChunkManager, chunkX: Int, chunkZ: Int, random: NukkitRandom, chunk: FullChunk) {
		...
		for (i in 0 until amount) {
			...
			ObjectSwampTree().generate(level, random, v.setComponents(x.toDouble(), y.toDouble(), z.toDouble()))
		}
	}
