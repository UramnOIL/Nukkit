...
class ItemMinecartHopper @JvmOverloads constructor(meta: Int? = 0, count: Int = 1) : Item(ItemID.Companion.MINECART_WITH_HOPPER, meta, count, "Minecart with Hopper") {
	...
	override fun onActivate(level: Level, player: Player, block: Block, target: Block, face: BlockFace, fx: Double, fy: Double, fz: Double): Boolean {
		if (Rail.isRailBlock(target)) {
			...
			val minecart = Entity.createEntity("MinecartHopper",
