...
class AddEntityPacket : DataPacket() {
	...
	var attributes: Array<Attribute?>? = arrayOfNulls<Attribute?>(0)
	var links: Array<EntityLink?>? = arrayOfNulls<EntityLink?>(0)
	...
	override fun encode() {
		...
		this.putAttributeList(attributes)
		this.put(Binary.writeMetadata(metadata))
		this.putUnsignedVarInt(links!!.size)
		for (link in links!!) {
			putEntityLink(link)
		}
	}
	...
	companion object {
		val NETWORK_ID: Byte = ProtocolInfo.ADD_ENTITY_PACKET
		var LEGACY_IDS: ImmutableMap<Integer?, String?>? = ImmutableMap.< Integer, String>builder<Integer?, String?>()
