 ...
 */
class BlockStemPumpkin @JvmOverloads constructor(meta: Int = 0) : BlockCrops(meta) {
	override val id: Int
		get() = BlockID.Companion.PUMPKIN_STEM
			...
			if (random.nextRange(1, 2) == 1) {
				if (this.damage < 0x07) {
					...
					for (face in Plane.HORIZONTAL) {
						...
						if (b.id == BlockID.Companion.PUMPKIN) {
							return Level.BLOCK_UPDATE_RANDOM
						}
					}
					...
					if (side.id == BlockID.Companion.AIR && (d.id == BlockID.Companion.FARMLAND || d.id == BlockID.Companion.GRASS || d.id == BlockID.Companion.DIRT)) {
						val ev = BlockGrowEvent(side, Block.Companion.get(BlockID.Companion.PUMPKIN))
						Server.instance!!.pluginManager.callEvent(ev)
						if (!ev.isCancelled) {
	...
	override fun toItem(): Item? {
		return ItemSeedsPumpkin()
	}

	override fun getDrops(item: Item): Array<Item?> {
		val random = NukkitRandom()
		return arrayOf(
				ItemSeedsPumpkin(0, random.nextRange(0, 3))
		)
	}
}